...
FF_BUILD_PREFIX="$FF_BUILD_ROOT/build/ffmpeg-$FF_ARCH/output"
...
FF_CC="$IJK_CC"
FF_CXX="$IJK_CXX"
FF_CFLAGS="-arch $FF_ARCH $IJK_CFLAGS"
FF_LDFLAGS="$IJK_LDFLAGS"
FF_DEP_LIBS=""
#ffmpeg options
FF_CFG_FLAGS="--arch=$IJK_ARCH"
...
if [ "$FF_PLATFORM" = "ios" -o "$FF_PLATFORM" = "osx" ]; then
    EXTRA_FLAGS="$EXTRA_FLAGS --enable-cross-compile"
    if [ "$FF_PLATFORM" = "ios" ]; then
        if [ "$FF_ARCH" = "x86_64" ]; then
            EXTRA_FLAGS="$EXTRA_FLAGS --disable-asm"
            EXTRA_FLAGS="$EXTRA_FLAGS --disable-mmx"
            EXTRA_FLAGS="$EXTRA_FLAGS --assert-level=2"
        fi
    else
        echo
    fi
    ...
    FF_CFG_FLAGS="$FF_CFG_FLAGS --pkgconfigdir=${FF_BUILD_PREFIX}/lib/pkgconfig"
    ...
    FF_CFLAGS="$FF_CFLAGS -ffast-math -fstrict-aliasing -Werror=strict-aliasing -Wall,--noexecstack"
    FF_CFLAGS="$FF_CFLAGS -DANDROID -DNDEBUG"
fi

if [ "$FF_BUILD_OPT" = "debug" ]; then
    EXTRA_FLAGS="$EXTRA_FLAGS --disable-optimizations"
    EXTRA_FLAGS="$EXTRA_FLAGS --enable-debug"
    EXTRA_FLAGS="$EXTRA_FLAGS --disable-small"
else
    EXTRA_FLAGS="$EXTRA_FLAGS --enable-optimizations"
    EXTRA_FLAGS="$EXTRA_FLAGS --disable-debug"
    EXTRA_FLAGS="$EXTRA_FLAGS --enable-small"
fi
FF_CFG_FLAGS="$FF_CFG_FLAGS $EXTRA_FLAGS"
...
# set prefix
FF_CFG_FLAGS="$FF_CFG_FLAGS --prefix=$FF_BUILD_PREFIX"
...
if [ "$FF_PLATFORM" = "android" ]; then
    ...
    cd $FF_BUILD_PREFIX
    rm -rf shared
    mkdir -p shared/lib/pkgconfig
    ln -s include shared/include
    ln -s libijkffmpeg.so shared/lib/libijkffmpeg.so
    cp lib/pkgconfig/*.pc shared/lib/pkgconfig
    cd -
    for f in $FF_BUILD_PREFIX/lib/pkgconfig/*.pc; do
        [ ! -f $f ] && continue
        cp $f $FF_BUILD_PREFIX/shared/lib/pkgconfig
        f=$FF_BUILD_PREFIX/shared/lib/pkgconfig/`basename $f`
